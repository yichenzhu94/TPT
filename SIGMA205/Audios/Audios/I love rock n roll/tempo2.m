function [t,xcr,D,onsetenv,oeFs] = tempo2(d,Fs,tmean,tsd,debug)
% [t,xcr,D,onsetenv,oeFs] = tempo(d,Fs,tmean,tsd,debug)
%    Estimate the overall tempo of a track for the MIREX McKinney
%    contest.
%    d is the input audio at sampling rate Fs.  tmean is the mode
...
%    oeFs is the sampling rate of onsetenv and D.
%
...
if length(onsetenv) == 0
  ...
  if (Fs ~= Fso)
    gg = gcd(Fso,Fs);
    d = resample(d,Fso/gg,Fs/gg);
    Fs = Fso;
  end

  D = spectrogram(d,swin,swin-shop,swin,Fs);

  % Construct db-magnitude-mel-spectrogram
  mlmx = fft2melmx(swin,Fs,nmel);
...
maxd = 60;
maxt = 120; % sec
maxcol = min(round(maxt*oeFs),length(onsetenv));
mincol = max(1,maxcol-round(maxd*oeFs));
...
%%[vv,spix] = min(abs(60./(startpd/oeFs) - 120));
...
t = [60/(startpd/oeFs) 60/(startpd2/oeFs) pratio];
